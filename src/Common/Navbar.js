        ...
        <nav className="sticky min-h-fit px-4 sm:px-14 lg:px-20">
            <div className="flex flex-wrap sm:flex-nowrap items-center py-3">
                ...
                <div className="hidden sm:flex justify-around justify-items-center sm:pr-10 lg:pr-30">
                    {links.map((link) => {
                        return (
                            <a href={link.href} className="sm:ml-8 px-1 md:text-lg hover:underline hover:decoration-accentprimary hover:decoration-2">{link.name}</a>
                        );
                    })}
                </div>
                ...
                <Menu>
                    {({ open }) => (
                        <>
                            ...
                            <Menu.Button><FontAwesomeIcon className={"sm:hidden mr-4 w-5 h-5 align-middle hover:opacity-75"} icon={open ? faXmark : faBars}/></Menu.Button>
                            <hr />
