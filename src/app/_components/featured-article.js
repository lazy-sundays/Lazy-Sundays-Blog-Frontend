...
export default async function FeaturedArticle() {
  async function getFeaturedArticleInfo() {
    const res = await fetch(
      process.env.STRAPI_URI_ROOT +
        "/api/featured-article?populate[article][populate]=*",
      {
        method: "GET",
        headers: {
  ...
  const articleInfo = (await getFeaturedArticleInfo()).data.article;

  return (
    <article className="text-center pt-8">
      <meta
        name="author"
        content={articleInfo.authors.map((author) => `${author.name}`)}
      ></meta>
      <ArticleHeader
      ...
      <LinkButton
        ...
        href={`/articles/${articleInfo.slug}`}
